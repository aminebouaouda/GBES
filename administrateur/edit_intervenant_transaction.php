...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Get the form data
    $facture_i_id = $_POST["facture_i_id"];
    ...
    $sql_update_cheque = "UPDATE cheque SET numero_cheque = '$numero_cheque' WHERE cheque_id = (SELECT cheque_id FROM facture_i WHERE facture_i_id = '$facture_i_id')";
    if ($conn->query($sql_update_cheque) === TRUE) {
        // Update the "cheque_id" in the "facture_i" table
        $sql_update_facture_cheque = "UPDATE facture_i SET cheque_id = (SELECT cheque_id FROM cheque WHERE cheque_id = (SELECT cheque_id FROM facture_i WHERE facture_i_id = '$facture_i_id')) WHERE facture_i_id = '$facture_i_id'";
        if ($conn->query($sql_update_facture_cheque) === TRUE) {
            // Next, update the transaction data in the "facture_i" table
            $sql_update_facture = "UPDATE facture_i SET date_facture = '$date_facture', mode_paiement = '$mode_paiement' WHERE facture_i_id = '$facture_i_id'";
            if ($conn->query($sql_update_facture) === TRUE) {
                // Update the details of the transaction in the "produit" table
                $sql_update_produit = "UPDATE produit SET ref = '$ref', libelle = '$libelle', quantite = '$quantite', prix_unitaire = '$prix_unitaire', solde = '$solde' WHERE produit_id = (SELECT produit_id FROM detail_i WHERE facture_i_id = '$facture_i_id')";
...
if (isset($_GET["facture_i_id"])) {
    $facture_i_id = $_GET["facture_i_id"];

    // Get the transaction data from the database
    $sql = "SELECT f.facture_i_id, f.date_facture, f.mode_paiement, c.numero_cheque, p.ref, p.libelle, p.quantite, p.prix_unitaire, p.solde
            FROM facture_i f
            LEFT JOIN cheque c ON f.cheque_id = c.cheque_id
            INNER JOIN detail_i di ON f.facture_i_id = di.facture_i_id
            INNER JOIN produit p ON di.produit_id = p.produit_id
            WHERE f.facture_i_id = '$facture_i_id'";
    ...
    if ($result->num_rows == 1) {
        ...
        // If no transaction is found with the given facture_i_id, redirect back to the transaction history page
        header("Location: intervenant_historique.php");
        exit();
    }
}
...
<body>
    ...
    <div class="edit-transaction-form">
        ...
        <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
            <input type="hidden" name="facture_i_id" value="<?php echo $facture_i_id; ?>">
            ...
            <a href="intervenant_historique.php" class="btn-cancel">Annuler</a>
        </form>
    </div>
