...
if (isset($_GET["facture_i_id"])) {
    ...
    $transactionDetails = getTransactionDetails($conn, $facture_i_id);

    if ($transactionDetails === null) {
        // Redirect to the main page if the transaction ID is not found
        header("Location: intervenant_facture.php");
        exit();
    }
} else {
...
function getTransactionDetails($conn, $facture_i_id)
{
    ...
    if ($result) {
        if ($result->num_rows > 0) {
            ...
            echo "No transaction found with ID: $facture_i_id";
            return null;
        }
    } else {
...
function generatePDF($transactionDetails, $total_amount)
{
    ...
    $pdf->SetCreator('GBES');
    $pdf->SetAuthor('GBES');
    $pdf->SetTitle('Facture Intervenant N°: ' . $transactionDetails["facture_i_id"]);
    $pdf->SetSubject('Facture Intervenant');
    ...
    .divider {
        ...
        <div class="facture">
            ...
            <img src="images/logo-color.png" alt="Your Company Logo">
                <h1>Facture  N°: ' . $transactionDetails["facture_i_id"] . '</h1>
            </div>
        ...
        <p><strong>Nom du intervenant:</strong> ' . htmlspecialchars($transactionDetails["intervenant_nom"] . ' ' . $transactionDetails["intervenant_prenom"]) . '</p>
        </div>
    ...
    $pdf->Output('facture_intervenant.pdf', 'D');
}
...
<head>
    ...
    <title>Facture Intervenant</title>
    <style>
        /* Common Styles */
        body {
            font-family: Arial, sans-serif;
        ...
        .intervenant-info {
            float: left;
            width: 50%;
            /* Add some space between the logo and the intervenant info */
            margin-top: 170px;
        }
...
<body>
    <div class="facture">
        ...
        <img src="images/logo-color.png" alt="Your Company Logo">
            <h1>Facture  N°:
                <?php echo $transactionDetails["facture_i_id"]; ?>
            </h1>
        </div>
        ...
        <div class="intervenant-info">
            <p><strong>Nom de l'intervenant:</strong>
                <?php echo htmlspecialchars($transactionDetails["intervenant_nom"] . ' ' . $transactionDetails["intervenant_prenom"]); ?>
            </p>
        </div>
    ...
    <a href="?facture_i_id=<?php echo $facture_i_id; ?>&generate_pdf=1">
